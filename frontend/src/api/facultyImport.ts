/**
 * API client for Faculty CSV Import
 */
...
export async function downloadTemplate(): Promise<Blob> {
  const response = await api.get('/api/admin/faculty/import/template/', {
    responseType: 'blob',
  })
...
export async function listImportJobs(): Promise<ImportJob[]> {
  const response = await api.get<ImportJob[]>('/api/admin/faculty/import/jobs/')
  return response.data
}
...
export async function getImportJob(id: string): Promise<ImportJob> {
  const response = await api.get<ImportJob>(
    `/api/admin/faculty/import/${id}/detail/`
  )
  return response.data
}
...
export async function downloadErrorReport(id: string): Promise<Blob> {
  const response = await api.get(
    `/api/admin/faculty/import/${id}/errors.csv/`,
    {
      responseType: 'blob',
    }
  )
