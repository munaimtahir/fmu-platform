            ...
            return Response(
                {
                    "error": {
                        ...
                        "details": {"courses_count": courses_count},
                    }
                },
                status=status.HTTP_400_BAD_REQUEST,
            )
...
class PeriodViewSet(viewsets.ModelViewSet):
    ...
    def students(self, request, pk=None):
        ...
        students = (
            Student.objects.filter(batch_id__in=matching_batch_ids, status="active")
            .select_related("batch", "group", "program")
            .order_by("batch__name", "group__name", "name")
        )
...
class LearningBlockViewSet(viewsets.ModelViewSet):
    queryset = (
        LearningBlock.objects.select_related("period", "track", "primary_department", "sub_department")
...
class ModuleViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        if block.block_type != LearningBlock.BLOCK_TYPE_INTEGRATED:
            ...
            raise ValidationError("Modules can only be added to INTEGRATED_BLOCK blocks")
        serializer.save()
...
class SectionViewSet(viewsets.ModelViewSet):
    queryset = Section.objects.select_related("course", "academic_period", "faculty", "group").all()
