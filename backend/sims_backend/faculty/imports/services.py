...
def resolve_department(department_name: str | None, row_num: int) -> tuple[Department | None, list[dict[str, str]]]:
    """
    ...
    if not department_name:
        errors.append({"column": "department_name", "message": "Department name is required"})
        return None, errors
...
def check_existing_faculty(email: str, mode: str) -> tuple[bool, FacultyProfile | None]:
    """
...
def check_duplicate_in_file(email: str, row_index: int, seen_emails: dict[str, int]) -> list[dict[str, str]]:
    ...
    if email:
        ...
        if email_lower in seen_emails:
            errors.append(
                {"column": "email", "message": f"Duplicate email '{email}' found in row {seen_emails[email_lower]}"}
            )
        else:
...
class FacultyImportService:
    ...
    def _create_faculty_user(
        name: str, email: str, department: Department | None = None, password: str | None = None
    ) -> tuple[User, bool]:
        """
    ...
    def preview(file, user, mode: str = FacultyImportJob.MODE_CREATE_ONLY) -> dict[str, Any]:
        ...
        duplicate_job = (
            FacultyImportJob.objects.filter(file_hash=file_hash, mode=mode, status=FacultyImportJob.STATUS_COMMITTED)
        ...
        for idx, row in enumerate(rows):
            ...
            if department and not errors:  # Only add generated fields if department is resolved and no critical errors
                ...
                generated_password = (
                    provided_password if provided_password else FacultyImportService._generate_password()
                )
