...
class TestChallanAccess:
    def setup_method(self):
        ...
        # Create a Charge
        self.charge = Charge.objects.create(title="Tuition Fee", amount=1000, due_date="2024-06-01")

        # Create Ledger Items
        self.ledger1 = StudentLedgerItem.objects.create(student=self.student1, charge=self.charge, status="PENDING")
        self.ledger2 = StudentLedgerItem.objects.create(student=self.student2, charge=self.charge, status="PENDING")

        # Setup challans
        self.challan1 = Challan.objects.create(
            student=self.student1, amount_total=1000, status="PENDING", challan_no="CH001", ledger_item=self.ledger1
        )

        self.challan2 = Challan.objects.create(
            student=self.student2, amount_total=1000, status="PENDING", challan_no="CH002", ledger_item=self.ledger2
        )
