...
def setup_data(db):
    ...
    att1 = Attendance.objects.create(
        session=session, student=student1, status="PRESENT", marked_by=admin_user, marked_at=now
    )
    att2 = Attendance.objects.create(
        session=session, student=student2, status="ABSENT", marked_by=admin_user, marked_at=now
    )
