    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            if not user.is_superuser:
                self.stdout.write(
                    self.style.WARNING(f"⚠️  User {username} exists but is not a superuser. Making it a superuser...")
                )
                ...
                try:
                    ...
                    if status == 200:
                        ...
                        try:
                            ...
                            pass
                        other_errors.append(
                            (f"{app_label_inner}.{model_name} (List)", list_url, f"{status}{error_detail}")
                        )
        ...
        # Final statistics
        total_tested = (
            len(working_urls) + len(error_500_urls) + len(error_403_urls) + len(error_404_urls) + len(other_errors)
        )
        self.stdout.write("\n" + "=" * 80)
        self.stdout.write("\n📈 Statistics:")
