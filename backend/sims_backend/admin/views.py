...
def admin_dashboard(request):
    ...
    recent_attendance = Attendance.objects.filter(marked_at__gte=seven_days_ago)
    ...
    recent_attendance.filter(status=Attendance.STATUS_PRESENT).count()
    ...
    recent_sessions = Session.objects.filter(created_at__gte=seven_days_ago)
...
class AdminUserViewSet(viewsets.ModelViewSet):
    ...
    def search(self, request):
        ...
        users = (
            ...
            .exclude(groups__name__in=["ADMIN", "Admin"])
            .distinct()[:20]
        )  # Limit to 20 results
