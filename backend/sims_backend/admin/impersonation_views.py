...
def start_impersonation(request):
    ...
    if not target_user.is_active:
        return Response({"error": "Cannot impersonate inactive users"}, status=status.HTTP_400_BAD_REQUEST)

    # Disallow impersonating other admins (recommended default)
    if target_user.is_superuser or in_group(target_user, "ADMIN"):
        return Response({"error": "Cannot impersonate admin users"}, status=status.HTTP_403_FORBIDDEN)
