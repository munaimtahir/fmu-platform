...
class DemoScenariosCommandTest(TestCase):
    ...
    def test_command_creates_20_students(self):
        ...
        demo_students = Student.objects.filter(reg_no__contains="DEMO_")
    ...
    def test_command_idempotency(self):
        ...
        call_command("seed_demo_scenarios", "--students", "20")
        first_count = Student.objects.filter(reg_no__contains="DEMO_").count()

        call_command("seed_demo_scenarios", "--students", "20")
        second_count = Student.objects.filter(reg_no__contains="DEMO_").count()
    ...
    def test_reset_flag_deletes_demo_objects(self):
        ...
        # Create initial data
        call_command("seed_demo_scenarios", "--students", "20")
        initial_count = Student.objects.filter(reg_no__contains="DEMO_").count()
        self.assertGreater(initial_count, 0)

        # Reset and recreate
        call_command("seed_demo_scenarios", "--students", "20", "--reset")
        final_count = Student.objects.filter(reg_no__contains="DEMO_").count()
    ...
    def test_results_published_scenario(self):
        ...
        published_results = ResultHeader.objects.filter(
            status=ResultHeader.STATUS_PUBLISHED, student__reg_no__contains="DEMO_"
        )
    ...
    def test_low_attendance_scenario(self):
        ...
        for student in demo_students:
            ...
            if attendances.count() > 0:
                ...
                if 0 < attendance_percent < 75:
                    low_attendance_found = True
                    break

        # We should have at least one student with low attendance
        self.assertTrue(low_attendance_found, "No student found with low attendance (< 75%)")
    ...
    def test_fees_voucher_generated_scenario(self):
        ...
        demo_students = Student.objects.filter(reg_no__contains="DEMO_")
        students_with_vouchers = Voucher.objects.filter(student__in=demo_students, status=Voucher.STATUS_GENERATED)
    ...
    def test_command_with_custom_parameters(self):
        ...
        call_command(
            "seed_demo_scenarios", "--students", "20", "--program", "BDS", "--term", "Block-2", "--sections", "2"
        )

        # Verify program was created
        from sims_backend.academics.models import Program

        program = Program.objects.filter(name="BDS").first()
