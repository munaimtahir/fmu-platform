...
def intake_upload_path(instance, filename):
    ...
    identifier = (
        ...
        if hasattr(instance, "submission_id") and instance.submission_id
        else (f"temp-{instance.pk}" if instance.pk else "temp")
    )
...
class StudentIntakeSubmission(models.Model):
    ...
    submission_id = models.CharField(
        max_length=50, unique=True, db_index=True, help_text="Unique submission reference (STU-YYYYMMDD-XXXX)"
    )
    status = models.CharField(
        max_length=20, choices=STATUS_CHOICES, default="PENDING", db_index=True, help_text="Current verification status"
    )
    ...
    cnic_or_bform = models.CharField(max_length=20, db_index=True, help_text="CNIC or B-Form number (normalized)")
    mobile = models.CharField(max_length=20, db_index=True, help_text="Mobile phone number (required)")
    email = models.EmailField(db_index=True, help_text="Email address (required)")
    address = models.TextField()

    # B) Guardian Information (ALL REQUIRED)
    guardian_name = models.CharField(max_length=200)
    guardian_relation = models.CharField(max_length=20, choices=GUARDIAN_RELATION_CHOICES)
    guardian_phone_whatsapp = models.CharField(max_length=20, help_text="Guardian WhatsApp number (required)")

    # C) Admission / Merit Details (ALL REQUIRED)
    mdcat_roll_number = models.CharField(
        max_length=50, db_index=True, help_text="MDCAT Roll Number (required, checked for duplicates)"
    )
    merit_number = models.IntegerField(validators=[MinValueValidator(1)], help_text="Merit number (must be > 0)")
    ...
    passing_year = models.IntegerField(validators=[MinValueValidator(1900), MaxValueValidator(2100)])
    total_marks_or_grade = models.CharField(max_length=50)
    obtained_marks_or_grade = models.CharField(max_length=50)
    subjects = models.CharField(max_length=500, help_text="Subjects studied (short text)")

    # E) Documents
    # MANDATORY
    passport_size_photo = models.ImageField(
        upload_to=intake_upload_path, help_text="Passport-size photograph (jpg/png only, max 1MB, REQUIRED)"
    )
    ...
    def clean(self):
        ...
        if self.cnic_or_bform:
            ...
            if not (11 <= len(self.cnic_or_bform) <= 13):
                raise ValidationError({"cnic_or_bform": "CNIC/B-Form must be 11-13 digits."})

        # Validate mobile number (basic)
        if self.mobile:
            mobile_digits = re.sub(r"[^\d]", "", self.mobile)
            if len(mobile_digits) < 10 or len(mobile_digits) > 15:
                raise ValidationError({"mobile": "Mobile number must be 10-15 digits."})
        ...
        if self.guardian_phone_whatsapp:
            ...
            if len(guardian_digits) < 10 or len(guardian_digits) > 15:
                raise ValidationError({"guardian_phone_whatsapp": "Guardian WhatsApp number must be 10-15 digits."})
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.submission_id:
            ...
            last_submission = (
                StudentIntakeSubmission.objects.filter(submission_id__startswith=f"STU-{date_str}-")
    ...
    def check_duplicates(self):
        ...
        cnic_duplicates = StudentIntakeSubmission.objects.filter(cnic_or_bform=cnic_normalized).exclude(
        ...
        # Check Mobile
        if self.mobile:
            mobile_duplicates = StudentIntakeSubmission.objects.filter(mobile=self.mobile).exclude(
                pk=self.pk if self.pk else None
            )
        ...
        # Check Email
        if self.email:
            email_duplicates = StudentIntakeSubmission.objects.filter(email=self.email).exclude(
                pk=self.pk if self.pk else None
            )
        ...
        if self.mdcat_roll_number:
            mdcat_duplicates = StudentIntakeSubmission.objects.filter(mdcat_roll_number=self.mdcat_roll_number).exclude(
                pk=self.pk if self.pk else None
            )
