...
- **Full attendance/results history.** This session ships Attendance/Results as home-payload-only (summary + latest 5), per explicit user decision. A confirmed-reusable, already-student-scoped, paginated path exists for a fast-follow: `GET /api/attendance/?student=<id>&ordering=-marked_at` and `GET /api/results/?student=<id>&ordering=-created_at` (both behind DRF `PageNumberPagination`, `{count,next,previous,results}`). `ResultHeaderViewSet` also raises a `403 FINANCE_BLOCKED` for finance-gated students — a real backend-enforced rule the client must surface, not bypass, when this history view is built.
- **Instrumented Compose UI tests were not added.** Blocked on a pre-existing gap: the project has no Hilt test-runner setup (`HiltTestApplication`/`@HiltAndroidTest`), so screens using `hiltViewModel()` can't be tested in isolation, and the existing `LoginScreenTest.kt` pattern (launching a bare `ComponentActivity`) doesn't actually exercise real navigation. Fixing this is a separate, scoped piece of work, not done here.
- **CI (`android-ci.yml`) stays `workflow_dispatch`-only** — user explicitly declined adding push/PR triggers this session.
- Remaining original spec items, still not started: Full state hardening for genuinely offline/500 conditions beyond what was tested, `bundleRelease`/Play-track promotion for this new build, and multi-API-level/tablet-layout emulator coverage (only API 36 phone-size was tested).
...
8. ~~Android core student screens (§1)~~ — **DONE 2026-09-11**: Home/Timetable/Attendance/Results screens, real navigation, and 401/session-expiry handling implemented and verified live against production; a real pre-existing sign-out infinite-loop bug was found and fixed along the way. Remaining: full attendance/results history (fast-follow, endpoints already confirmed reusable), instrumented UI tests (blocked on missing Hilt test-runner setup), `bundleRelease`/Play-track promotion.
